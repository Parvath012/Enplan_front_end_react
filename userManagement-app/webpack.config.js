...
process.env.REACT_APP_USER_MANAGEMENT_API_URL = process.env.REACT_APP_USER_MANAGEMENT_API_URL || 'http://172.16.20.116:8081';
process.env.REACT_APP_USER_HIERARCHY_API_URL = process.env.REACT_APP_USER_HIERARCHY_API_URL || 'http://172.16.20.116:8881';
process.env.REACT_APP_USER_MANAGEMENT_URL = process.env.REACT_APP_USER_MANAGEMENT_URL || 'http://localhost:3006';
process.env.COMMON_APP_URL = process.env.COMMON_APP_URL || 'http://localhost:3002/remoteEntry.js';

// Validate required environment variables for userManagement-app
const requiredEnvVars = [
  'REACT_APP_USER_MANAGEMENT_API_URL',
  'REACT_APP_USER_HIERARCHY_API_URL',
  'REACT_APP_USER_MANAGEMENT_URL',
...
if (missing.length > 0) {
  console.warn(`⚠️ User Management App: Some environment variables are missing, using defaults:`);
  missing.forEach(varName => console.warn(`   - ${varName}`));
  console.warn(`\nConsider creating a .env file for custom configuration.`);
} else {
  console.log(`✓ User Management App: All required environment variables are present`);
}
